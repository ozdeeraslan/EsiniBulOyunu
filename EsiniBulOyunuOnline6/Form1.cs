namespace EsiniBulOyunuOnline6
{
    public partial class Form1 : Form
    {
        ...
        int boyut = 2; // satir ve sütun sayisi
        ...
        {
            int bosluk = 10;
            int genislik = (pnlKartlar.Width - ((boyut - 1) * bosluk)) / boyut;
            int yukseklik = (pnlKartlar.Height - ((boyut - 1) * bosluk)) / boyut;
            ...
            for (int y = 0; y < boyut; y++)
            {
                for (int x = 0; x < boyut; x++)
                {
                    ...
                    resimKutusu.Size = new Size(genislik, yukseklik);
        ...
        // sender: tiklanan Resim Kutusunu tasir.
        private void ResimKutusu_Click(object sender, EventArgs e)
        {
            PictureBox tiklanan = (PictureBox)sender;

            // eger ayni karta 2.kez tiklandiysa bir sey yapmadan cik
            if (aciklar.Count == 1 && aciklar[0] == tiklanan)
                return;

            // yeni bir resim kutusuna tiklandiginda önceden acilanlarin kapatilmasi
            if (aciklar.Count == 2)
            {
            ...
            // 2.karti actiginda yapilacak eslesme kontrolü
